"""Diagnostic test harness for the Fourier-domain / JPEG-ghost engine.
...
Usage:
    python3 test_ghost_engine_diagnostic.py [--tag before|after]
...
from ghost_engine.contracts import EngineInput, EngineOutput, ImageMetadata
from ghost_engine.engine import GhostEngine
...
OUTPUT_ROOT = (
    "/home/kingsley-judewin/dsp project/"
    "imagedetectionengine/test results/ghost_engine"
)
...
def run_engine_safely(engine: GhostEngine,
                      image: np.ndarray,
...
def _candidate_geometry(labels: np.ndarray) -> tuple:
    """Measure the two properties the engine's validity gate tests.

    Args:
        labels: Integer 0/1 class labels over the analysis region.

    Returns:
        Tuple of (class-0 area fraction, largest-component coherence).
    """
    ghost_mask = (labels == 0).astype(np.uint8)
    area = int(ghost_mask.sum())
    if area == 0:
        return 0.0, 0.0
    count, _, stats, _ = cv2.connectedComponentsWithStats(ghost_mask,
                                                          connectivity=8)
    if count <= 1:
        return area / float(labels.size), 0.0
    largest = float(stats[1:, cv2.CC_STAT_AREA].max())
    return area / float(labels.size), largest / float(area)


def deep_probe(image: np.ndarray, metadata: ImageMetadata,
               output: EngineOutput) -> dict:
    """Collect the internals the verdict rests on, including what the gate hid.

    The engine's Bhattacharyya statistic is zeroed for any candidate failing the
    SE-MinCut-substitute validity test, so a D_max of exactly 0 says only "no
    candidate survived" and not why. This probe re-runs the sweep at the
    unshifted grid position with the gate REMOVED, recording every candidate's
    ungated B together with the two geometric quantities the gate judges - class-0
    area fraction and largest-component coherence. That distinguishes an engine
    correctly rejecting fractal noise from one discarding a genuine ghost.

    Args:
        image: BGR uint8 image.
        metadata: Real metadata for this image.
        output: The EngineOutput already produced for this image, reused rather
            than recomputed so the probe costs one grid shift instead of a
            second full sweep.

    Returns:
        Dictionary of the internal quantities.
    """
    from ghost_engine import constants as ghost_constants
    from ghost_engine.computer import GhostDetector
    from ghost_engine.preprocessor import GhostPreprocessor

    steps = {step["name"]: step["key_values"]
             for step in output.computation_steps}
    probe = {
        "steps": steps,
        "flagged_regions": (0 if output.flagged_regions is None
                            else len(output.flagged_regions)),
    }

    try:
        preprocessor = GhostPreprocessor()
        detector = GhostDetector(preprocessor)
        prepared = preprocessor.prepare(image, metadata)
        qualities = list(range(ghost_constants.QUALITY_FACTOR_MINIMUM,
                               ghost_constants.QUALITY_FACTOR_MAXIMUM + 1,
                               ghost_constants.DEFAULT_QUALITY_FACTOR_STEP))
        energies = np.stack([
            detector.difference_energy(
                prepared.colour_rgb,
                preprocessor.recompress(prepared.colour_rgb, quality))
            for quality in qualities])
        normalised = detector.normalise_across_qualities(energies)

        rows = []
        for index, quality in enumerate(qualities):
            region = detector._crop_to_analysis_region(  # noqa: SLF001
                normalised[index], 0, 0, prepared.colour_rgb.shape[:2])
            if region.size == 0:
                continue
            labels = detector.segment_difference_map(region)
            statistics = detector._class_statistics(region, labels)  # noqa: SLF001
            ungated = (0.0 if statistics is None else
                       detector._bhattacharyya_from_statistics(*statistics))  # noqa: SLF001
            area_fraction, coherence = _candidate_geometry(labels)
            rows.append({
                "q2": quality,
                "ungated_B": round(float(ungated), 4),
                "area_fraction": round(float(area_fraction), 4),
                "coherence": round(float(coherence), 4),
                "passes_gate": bool(detector.is_valid_ghost_candidate(labels)),
            })

        best = max(rows, key=lambda row: row["ungated_B"]) if rows else {}
        survivors = [row for row in rows if row["passes_gate"]]
        too_large = sum(1 for row in rows if row["area_fraction"]
                        > ghost_constants.MAXIMUM_GHOST_AREA_FRACTION)
        too_small = sum(1 for row in rows if row["area_fraction"]
                        < ghost_constants.MINIMUM_GHOST_AREA_FRACTION)
        incoherent = sum(1 for row in rows if row["coherence"]
                         < ghost_constants.MINIMUM_GHOST_SPATIAL_COHERENCE)
        probe["ungated_shift00"] = {
            "candidates": rows,
            "max_ungated_B": best.get("ungated_B"),
            "max_ungated_at_q2": best.get("q2"),
            "gate_survivors": len(survivors),
            "rejected_area_too_large": too_large,
            "rejected_area_too_small": too_small,
            "rejected_incoherent": incoherent,
            "median_area_fraction": (round(float(np.median(
                [row["area_fraction"] for row in rows])), 4) if rows else None),
            "median_coherence": (round(float(np.median(
                [row["coherence"] for row in rows])), 4) if rows else None),
        }
    except Exception as error:  # noqa: BLE001 - probe must never kill the run
        probe["ungated_shift00"] = {"error": f"{type(error).__name__}: {error}"}
    return probe
...
def save_report_text(report: dict, output_path: str) -> None:
    ...
    lines = [
        "=" * 74,
        f"GHOST ENGINE DIAGNOSTIC REPORT  [{report['tag']}]",
...
def engine_sweep_settings() -> str:
    """Describe the sweep the engine is configured to run.

    Returns:
        One-line description of the pruned (q2, dx, dy) search space.
    """
    from ghost_engine import constants as ghost_constants
    qualities = len(range(ghost_constants.QUALITY_FACTOR_MINIMUM,
                          ghost_constants.QUALITY_FACTOR_MAXIMUM + 1,
                          ghost_constants.DEFAULT_QUALITY_FACTOR_STEP))
    shifts = len(range(0, ghost_constants.GRID_SHIFT_MAXIMUM + 1,
                       ghost_constants.DEFAULT_GRID_SHIFT_STEP)) ** 2
    return (f"{qualities} quality factors x {shifts} grid shifts = "
            f"{qualities * shifts} of the paper's "
            f"{ghost_constants.FULL_SEARCH_COMBINATION_COUNT} combinations")
...
def analyze_result(engine_output: EngineOutput,
    ...
    ghost = next((v for k, v in steps.items() if "Pipeline B" in k), {})
    resampling = next((v for k, v in steps.items() if "Pipeline A" in k), {})
    ungated = probe.get("ungated_shift00", {})
    distance = ghost.get("D_max")
    threshold = ghost_constants.BHATTACHARYYA_THRESHOLD

    reasoning = (
        f"Engine reported raw_score={engine_output.raw_score:.6f} = "
        f"clip(D_max/Th, 0, 1) with D_max={distance} against the paper's "
        f"Th={threshold}, at confidence {engine_output.confidence:.4f}, "
        f"is_reliable={engine_output.is_reliable}, with "
        f"{probe.get('flagged_regions')} flagged region(s). Pipeline B swept "
        f"{ghost.get('combinations_evaluated')} of "
        f"{ghost.get('full_search_size')} (q2, dx, dy) combinations and "
        f"rejected {ghost.get('degenerate_segmentations')} of them as "
        f"degenerate segmentations; the winning candidate placed the region's "
        f"estimated original quality at q2="
        f"{ghost.get('estimated_region_quality_factor')} with grid shift "
        f"{ghost.get('detected_grid_shift')}. With the validity gate removed "
        f"at the unshifted grid position the largest Bhattacharyya distance "
        f"available was {ungated.get('max_ungated_B')} at q2="
        f"{ungated.get('max_ungated_at_q2')}, from a candidate whose class-0 "
        f"region held a median {ungated.get('median_area_fraction')} of the "
        f"frame at median coherence {ungated.get('median_coherence')}; "
        f"{ungated.get('gate_survivors')} of those candidates passed the gate "
        f"({ungated.get('rejected_area_too_large')} rejected as covering more "
        f"than {ghost_constants.MAXIMUM_GHOST_AREA_FRACTION} of the frame, "
        f"{ungated.get('rejected_area_too_small')} as covering less than "
        f"{ghost_constants.MINIMUM_GHOST_AREA_FRACTION}, "
        f"{ungated.get('rejected_incoherent')} as spatially incoherent). "
        f"Pipeline A (auxiliary, unscorable) reported {resampling}."
    )
    mathematical_explanation = (
        f"This engine measures a double-quantization artefact in the 8x8 "
        f"block-DCT domain. Azarian-Pour Eq. 8 recompresses the image at each "
        f"candidate quality q2 and takes the RGB squared difference smoothed "
        f"over a w={ghost_constants.GHOST_SMOOTHING_WINDOW} window; a region "
        f"already quantized at q2 loses little further information, so Eq. 9's "
        f"per-pixel min-max normalisation across the sweep drives that region "
        f"toward zero while the rest of the frame stays high - the ghost. "
        f"Eq. 10's Bhattacharyya distance then measures how separable the "
        f"segmented ghost is from the background, and the maximum over all "
        f"(q2, dx, dy) is D_max, compared against Th={threshold}. For this "
        f"image D_max={distance}, so raw_score="
        f"{engine_output.raw_score:.6f} and the engine "
        f"{'cleared' if (distance or 0) > threshold else 'did not clear'} the "
        f"paper's own forged/authentic boundary. The decisive quantity is not "
        f"the arithmetic but the direction of the quality gap: the SKILL states "
        f"the method resolves a splice only when the pasted region's original "
        f"quality was LOWER than the re-save quality, exceeding 95% "
        f"sensitivity only once that gap passes "
        f"{ghost_constants.RELIABLE_QUALITY_FACTOR_GAP}, and neither quantity "
        f"is computable from the image alone."
    )
...
def run_case(engine: GhostEngine,
             image_folder: str,
             case_number: int,
             case_type: str,
             ground_truth: str,
    ...
    for index, (path, cv_image, pil_image) in enumerate(
        ...
        output = run["output"]
        probe = deep_probe(cv_image, metadata, output)
        analysed = analyze_result(output, facts, case_type, ground_truth,
                                  expectation, probe)

        stem = (f"ghost_case{case_number}_img{index}_"
...
def save_summary(report: dict, output_path: str) -> None:
    ...
    statistics = report["statistics"]
    lines = [
        "FOURIER / JPEG-GHOST ENGINE - DIAGNOSTIC SUMMARY",
...
def main() -> int:
    ...
    notices.append(
        f"Pipeline B ran the SKILL-sanctioned pruned sweep: "
        f"{engine_sweep_settings()}. Pipeline A ran its full 692-map search "
        f"set but cannot score, because this SKILL prints no numeric value "
        f"for its decision threshold rho_T.")
