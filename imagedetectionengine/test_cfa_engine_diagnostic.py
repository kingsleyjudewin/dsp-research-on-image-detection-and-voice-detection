"""Diagnostic test harness for the Cfa's-Law forgery-detection engine.
...
Usage:
    python3 test_cfa_engine_diagnostic.py [--tag before|after]
...
from cfa_engine.contracts import EngineInput, EngineOutput, ImageMetadata
from cfa_engine.engine import CfaEngine
...
OUTPUT_ROOT = (
    "/home/kingsley-judewin/dsp project/"
    "imagedetectionengine/test results/cfa_engine"
)
...
def run_engine_safely(engine: CfaEngine,
                      image: np.ndarray,
...
def deep_probe(image: np.ndarray, metadata: ImageMetadata) -> dict:
    """Re-run the engine with its condition gate bypassed, to see the maths.

    The CFA engine correctly refuses to run below the corpus's JPEG-quality
    floor, which means a gated image yields no numbers at all. For DIAGNOSIS we
    still need to know what Ferrara's pipeline would have produced, so this
    calls the engine with metadata whose quality factor is reported as
    uncompressed. The result is never presented as a verdict - it exists purely
    to show what the gate is suppressing.

    Args:
        image: BGR uint8 image.
        metadata: The real metadata for this image.

    Returns:
        Dictionary of the ungated internal quantities.
    """
    ungated = ImageMetadata(
        estimated_compression_level=0.0,
    ...
    try:
        output = CfaEngine().analyse(EngineInput(image=image, metadata=ungated))
    except Exception as error:  # noqa: BLE001
        return {"error": f"{type(error).__name__}: {error}"}

    steps = {step["name"]: step["key_values"] for step in output.computation_steps}
    return {
        "ungated_raw_score": float(output.raw_score),
        "ungated_probability": (None if output.probability is None
                                else float(output.probability)),
        "ungated_is_reliable": bool(output.is_reliable),
        "ungated_note": output.reliability_note,
        "ungated_flagged_regions": (0 if output.flagged_regions is None
...
def save_report_text(report: dict, output_path: str) -> None:
    ...
    lines = [
        "=" * 74,
        f"CFA ENGINE DIAGNOSTIC REPORT  [{report['tag']}]",
...
def constants_reduction_rule() -> str:
    """Name of the map-reduction rule the engine is configured to use.

    Returns:
        The rule name from the engine's constants module.
    """
    from cfa_engine import constants as cfa_constants
    return str(cfa_constants.MAP_REDUCTION_RULE)
...
def analyze_result(engine_output: EngineOutput,
    ...
    feature = steps.get("Feature extraction", {})
    posterior = steps.get("Posterior and mixture", {})
    grid = steps.get("Grid consistency", {})
    reasoning = (
        f"Engine reported raw_score={engine_output.raw_score:.6f} "
        f"(reduction rule '{constants_reduction_rule()}' over the per-block "
        f"tampering map) at confidence {engine_output.confidence:.4f}, "
        f"is_reliable={engine_output.is_reliable}. With the JPEG-quality gate "
        f"bypassed for diagnosis the same image yields raw_score="
        f"{probe.get('ungated_raw_score')}, probability="
        f"{probe.get('ungated_probability')}, "
        f"{probe.get('ungated_flagged_regions')} flagged region(s). "
        f"Feature stage: {feature}. Posterior/mixture stage: {posterior}. "
        f"Grid-consistency layer: {grid}."
    )
    mathematical_explanation = (
        f"Ferrara's pipeline extracts the green channel, forms the bilinear "
        f"prediction error e(x,y) of Eq. 9, takes its locally-weighted variance "
        f"over a Gaussian window of standard deviation K/2 restricted to each "
        f"lattice class (Eq. 10), and reduces each block to "
        f"L = log(GM_A/GM_I) (Eq. 11-12) - the log ratio of prediction-error "
        f"variance at acquired versus interpolated Bayer positions. Under M1 "
        f"(CFA intact) Eq. 13 requires this to be positive; under M2 (CFA "
        f"destroyed) Eq. 14 fixes its mean at zero. For this image the fitted "
        f"mixture reported {posterior}. The per-block posterior Pr(M1|L) of "
        f"Eq. 15-17 is then inverted to a tampering map and reduced to the "
        f"scalar above. The engine's own gate matters here more than the "
        f"arithmetic: the SKILL states demosaicing correlation is the dominant "
        f"casualty of JPEG compression, and this image's estimated quality "
        f"factor is {facts.estimated_quality_factor:.0f}."
    )
...
def run_case(engine: CfaEngine,
    ...
    for index, (path, cv_image, pil_image) in enumerate(
        ...
        stem = (f"cfa_case{case_number}_img{index}_"
