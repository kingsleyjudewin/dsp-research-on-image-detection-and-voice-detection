...
Usage:
    python3 test_geometry_engine_diagnostic.py [--tag before|after]
...
from geometry_engine.contracts import EngineInput, EngineOutput, ImageMetadata
...
OUTPUT_ROOT = (
    "/home/kingsley-judewin/dsp project/"
    "imagedetectionengine/test results/geometry_engine"
)
...
def run_engine_safely(engine: GeometryEngine,
                      image: np.ndarray,
...
def deep_probe(image: np.ndarray, metadata: ImageMetadata) -> dict:
    """Collect the internal quantities the verdict rests on.

    Unlike the compression-based engines there is no gate to bypass here: this
    engine's premise is geometric, not codec-related, so whatever it reports is
    what it computed. This flattens the computation trace and pulls out the
    vanishing-point and height-ratio internals a reader needs.
    ...
    """
    try:
        output = GeometryEngine().analyse(EngineInput(image=image,
                                                      metadata=metadata))
...
def save_report_text(report: dict, output_path: str) -> None:
    ...
    lines = [
        "=" * 74,
        f"GEOMETRY ENGINE DIAGNOSTIC REPORT  [{report['tag']}]",
...
def constants_reduction_rule() -> str:
    ...
    """
    from geometry_engine import constants as cfa_constants
...
def analyze_result(engine_output: EngineOutput,
    ...
    vanishing = next((v for k, v in steps.items() if "anishing" in k), {})
    ratios = next((v for k, v in steps.items() if "eight" in k or "atio" in k), {})
    scene = next((v for k, v in steps.items() if "cene" in k or "egion" in k), {})
    reasoning = (
        f"Engine reported raw_score={engine_output.raw_score:.6f} (1 - C_min "
        f"from Yao Eq. 8) at confidence {engine_output.confidence:.4f}, "
        f"is_reliable={engine_output.is_reliable}, with "
        f"{probe.get('flagged_regions')} flagged region(s). Scene preparation: "
        f"{scene}. Vanishing-point stage: {vanishing}. Height-ratio stage: "
        f"{ratios}."
    )
    mathematical_explanation = (
        f"This engine measures whether objects resting on a common ground "
        f"plane have image-space heights consistent with a single pinhole "
        f"projection. It estimates the reference plane's vanishing line v0, "
        f"then for each object pair computes Yao Eq. 7, the real-world height "
        f"ratio implied by the pair's image coordinates. Eq. 8 turns the gap "
        f"between that measured ratio and the expected ratio alpha into "
        f"C = 2*F(-|alpha-beta|; 0, sigma^2) with sigma = 0.1*alpha, and the "
        f"engine publishes 1 - C_min. For this image the vanishing-point stage "
        f"reported {vanishing} and the ratio stage {ratios}. The decisive "
        f"question is not the arithmetic but whether alpha was known: the SKILL "
        f"requires it from object-class knowledge, and with none available the "
        f"engine assumes 1.0, meaning it is testing whether every paired region "
        f"is the same real-world height."
    )
...
def run_case(engine: GeometryEngine,
             image_folder: str,
             case_number: int,
             case_type: str,
             ground_truth: str,
...
def save_summary(report: dict, output_path: str) -> None:
    ...
    statistics = report["statistics"]
    lines = [
        "GEOMETRY ENGINE - DIAGNOSTIC SUMMARY",
